...
def debug():
    """List all files Render can see — helps diagnose template location."""
    import os
    files = []
    for root, dirs, filenames in os.walk(str(BASE_DIR)):
        for f in filenames:
            files.append(os.path.join(root, f).replace(str(BASE_DIR), ""))
    return jsonify({
        "base_dir": str(BASE_DIR),
        "files": sorted(files),
        "ha0935": str(HA0935_TEMPLATE),
        "hv0713": str(HV0713_TEMPLATE),
        "ha0935_exists": HA0935_TEMPLATE.is_file(),
        "hv0713_exists": HV0713_TEMPLATE.is_file(),
    })
